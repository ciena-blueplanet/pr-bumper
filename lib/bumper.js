...
class Bumper {
  ...
  _getLastPr () {
    return exec('git log -10 --oneline').then((stdout) => {
      ...
      // we find that one
      let mergeCommit = __.find(commits, (commit) => {
      ...
      if (mergeCommit) {
        ...
        prNumber = mergeCommit.split(' ')[4].replace('#', '')
      } else {
        // Bitbucket cloud has it's own format for PR merge messages.
        // The commit that represents the merging of the PR will include the text 'pull request #' so
        // we find that one
        mergeCommit = __.find(commits, (commit) => {
          return commit.match(/pull request #/) !== null
        })
        prNumber = mergeCommit.match(/#(\d*)/)[1]
      }
