...
class Bumper {
  ...
  getMergedPrScope () {
    return this._getMergedPrInfo().then(info => {
      return Promise.resolve(info.scope)
    })
  }
  ...
  _parsePackagesInfo (packagesInfo = '[]') {
    return JSON.parse(packagesInfo)
  }
   ...
   * @returns {Promise} - a promise resolved when changelog has been prepended
   */
  _maybePrependChangelogForPackages (info, packagesInfo = []) {
    if (!this.config.isEnabled('changelog')) {
      logger.log('Skipping prepending changelog because of config option.')
      return Promise.resolve(info)
    }
    ...
    const {paths, promises} = packagesInfo.reduce(({promises, paths}, packageInfo) => {
      const version = this._getNextVersion(packageInfo.version, info.scope)
      const data = this._getChangelogText(version, dateString, info.changelog)
  ...
  _getNextVersion (currentVersion, scope) {
