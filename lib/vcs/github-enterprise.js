...
function getFetchOpts (config) {
  const readToken = config.vcs.auth.readToken
  const headers = {}
...
/**
 * VCS interface for private GitHub Enterprise
 *
 * @class
 * @implements {Vcs}
 */
class GitHubEnterprise {
  ...
  constructor (config) {
    ...
    this.baseUrl = `https://${domain}/api/v3`
  }
   ...
   */
  addRemoteForPush () {
    const ghToken = this.config.vcs.auth.writeToken
    ...
    return exec(`git remote add ci-origin https://${ghToken}@${domain}/${owner}/${repo}`)
      .then(() => {
        return 'ci-origin'
      })
  }
...
module.exports = GitHubEnterprise
