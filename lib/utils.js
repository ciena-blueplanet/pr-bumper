...
const utils = {
  ...
  isFeatureEnabled (config, feature) {
    return __.get(config, `features.${feature}.enabled`) || false
  },
  ...
  maybePostComment (config, vcs, msg, isError) {
    const areCommentsEnabled = this.isFeatureEnabled(config, 'comments')
    if (!process.env['SKIP_COMMENTS'] && config.computed.ci.isPr && areCommentsEnabled) {
  ...
  maybePostCommentOnError (config, vcs, func) {
    ...
    try {
      ...
      const areCommentsEnabled = this.isFeatureEnabled(config, 'comments')
      if (config.computed.ci.isPr && areCommentsEnabled) {
