...
const utils = {
  ...
  /**
   * Create a release branch if there is a major bump
   * @param {PrInfo} info - the pr info
   * @returns {PrInfo} the pr info object passed in
   */
  createReleaseBranch (info) {
    if (info.scope === 'major') {
      // use info.version
      exec(`git checkout -b  ${info.version}`, function (error, stdout, stderr) {
        if (error !== null) {
          console.log(`exec error: ${error}`)
        } else {
          exec(`git push origin ${info.version}`)
        }
      })
    }
    return info
