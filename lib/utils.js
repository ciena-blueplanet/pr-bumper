...
const utils = {
  ...
  getConfig (options) {
    ...
    return {
      owner,
      repo,
      commitShaRange: process.env[options.commitEnv],
  ...
  getSha (config) {
    if (config.isPr) {
      console.log(`Using CI commit sha range [${config.commitShaRange}]`)
      return Promise.resolve(config.commitShaRange.split('...').pop())
    }
