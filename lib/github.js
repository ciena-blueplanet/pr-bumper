...
class GitHub {
  ...
  push () {
    ...
    return exec(`git config credential.helper "store --file=~/.git/credentials"`)
      .then(() => {
        // TODO: find a safer way to do this, as the token can be displayed if a bug
        // is introduced here and exec errors out.
        return exec(`echo "https://${githubToken}@github.com" > ~/.git/credentials`)
      })
      .then(() => {
        return exec(`git push -q origin master`)
      })
      .then(() => {
        return exec(`git push -q origin --tags`)
      })
  }
}
