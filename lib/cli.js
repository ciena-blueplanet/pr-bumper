...
class Cli {
  ...
  run (cmd, skipComments, packagesJson) {
    ...
    if (cmd === 'bump') {
      ...
      return bumper.prependChangelog(packagesJson)
    } else if (cmd === 'get-merged-pr-scope') {
      return bumper.getMergedPrScope()
    } else if (cmd === 'get-last-merge-commit-hash') {
      return bumper.getLastMergeCommitHash()
    }
